...
const sequelize = new Sequelize('db_test', 'root', 'nttuan1995', {
	host: 'localhost',
	dialect: 'mysql',
	pool: {
		max: 5,
...
function routes(router) {
    ...
    await User.findOne({ where: { userName, password }, attributes: [ 'userName', 'password' ]})
      .then((user) => {
        if (user) {
          ...
          ctx.body = { status: 404, data: { error: { message: 'The ID or password you entered is incorrect' }} }
        }
      })
      .catch(err => {
        ctx.body = { status: 404, data: err }
      })
  })
  ...
  router.post('/signup', async ctx => {
    ...
    if (format.test(userName)) {
      ctx.body = { status: 404, data: { error: { message: "ID don't allow a special character or white space" }} }
    }
    payload = removeProperties(payload)
  ...
  router.post('/checkid', async ctx => {
    ...
    await User.findOne({ where: { userName }, attributes: [ 'userName' ]})
      .then((user) => {
        if (user) {
          ctx.body = { status: 404, data: { error: { message: 'ID already exists' }} }
        } else {
          ctx.body = { status: 200 }
        }
      })
  ...
  router.put('/changepw', async ctx => {
    ...
    .then(async user => {
      await user.update({
        ...
        ctx.body = { status: 404, data: { error: { message: 'Something were wrong' }} }        
      })
    })
    .catch(() => {
      ctx.body = { status: 404, data: { error: { message: 'Something were wrong' }} }
